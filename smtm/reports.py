...
def income_summary(txns: list[Transaction]) -> pd.DataFrame:
    ...
    rows = [{"month": t.month, "source": t.store_normalized or t.store_raw,
             "amount": t.amount} for t in income]
    df = pd.DataFrame(rows)
    pivot = df.pivot_table(
        index="month", columns="source", values="amount",
        aggfunc="sum", fill_value=0
    )
    pivot["TOTAL"] = pivot.sum(axis=1)
    return pivot.sort_index()
...
def category_averages(summary: pd.DataFrame) -> dict[str, float]:
    ...
    return {col: summary[col].mean() for col in sorted(cols, key=lambda c: -summary[c].mean())}
